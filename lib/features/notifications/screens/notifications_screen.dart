...
String _timeAgo(WidgetRef ref, DateTime dateTime) {
  ...
  if (diff.inHours < 1)
    return t(ref, 'minutesAgo', params: {'count': '${diff.inMinutes}'});
  if (diff.inDays < 1)
  ...
  Future<void> _handleClearAll(BuildContext context, WidgetRef ref) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        title: Text(
          t(ref, 'clearNotificationsConfirmTitle'),
          style: GoogleFonts.publicSans(fontWeight: FontWeight.w800),
        ),
        content: Text(
          t(ref, 'clearNotificationsConfirmBody'),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return PopScope(
      onPopInvokedWithResult: (didPop, result) {
        if (!didPop) return;
        // Viewing this list is itself the "read" action from the user's
        // perspective — requiring an explicit per-item tap or "mark all
        // read" left the Home badge stuck at the old count for anyone who
        // just glanced at the list and went back. Firing on leave (not on
        // open) still lets the unread dot/tint show while actually browsing.
...
class _NotificationCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      color: unread
          ? style.color.withValues(alpha: context.isDark ? 0.12 : 0.06)
          : context.cardFill,
      borderRadius: BorderRadius.circular(18),
      child: InkWell(
        borderRadius: BorderRadius.circular(18),
